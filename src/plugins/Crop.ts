...
export default class Crop extends Plugin {
  ...
  toolbarId = "react-img-editor-en-crop-toolbar" + uuid();
  ...
  onDraw = (drawEventParams: DrawEventParams) => {
    ...
    this.rect.dragBoundFunc((pos: any) => {
      ...
      if (this.transformer.width() >= 0) {
        if (pos.x <= 0) x = 0;
        if (pos.x >= stage.width() - this.transformer.width())
          x = stage.width() - this.transformer.width();
      } else {
      ...
      if (this.transformer.height() >= 0) {
        if (pos.y <= 0) y = 0;
        if (pos.y >= stage.height() - this.transformer.height())
          y = stage.height() - this.transformer.height();
      } else {
